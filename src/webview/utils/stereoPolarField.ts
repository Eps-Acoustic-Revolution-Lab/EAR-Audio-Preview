...
export function polarSampleFillAlpha(base: number, brightnessPct: number): number {
...
export function updatePolarRmsPeak(
  ...
  const fallScale =
    peakFallDbPerFrame > 0
      ? 1 - Math.pow(10, -peakFallDbPerFrame / 20)
      : 0;
...
export function polarDisplayMax(
  rms: Float32Array,
  peak: Float32Array,
): number {
  let m = 0;
